...
string String55(string text) { 
    ...
    for (int i = 0; i < text.size(); i++) {
        ...
        if ((simvol>="А" && simvol<="я") || simvol=="ё" || simvol=="Ё"){
            word += simvol;
        }
        else {
            if (word.size() > 0) {
...
string Str39(string text){
    ...
    if (res.empty()) return "Нет уникальных";
    return res;
}
...
string Str30(string n20) {
    ...
    for (int i = 0; i < n20.size(); i++) {
        ...
        if (!v) {
            return "Ошибка: недопустимый символ '" + s + "'";
        }
    }
    int res = 0;
    for (int i = 0; i < n20.size(); i++) {
...
string t63(string mes) {
    using namespace std;
    string let = "АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдежзийклмнопрстуфхцчшщъыьэюяABWGDEVZIJKLMNOPRSTUFHCQYXabwgdevzijklmnoprstufhcqyx0123456789";
    string cod[] = {
...
void Five5() {
    ...
    if (!fin.is_open()) {
        cout << "Ошибка: не могу открыть FN1.txt";
    }
    string prim;
    while (getline(fin, prim)) {
        ...
        if (oppos == -1) {
            fout << prim << " Ошибка: нет операции\n";
            cout << "Ошибка: нет операции\n";
            continue;
        }
        string num1 = clean.substr(0, oppos);
        string num2 = clean.substr(oppos + 1, ravpos - oppos - 1);
        if (num1.empty() || num2.empty()) {
            fout << prim << " Ошибка: пустое число\n";
            cout << "Ошибка: пустое число\n";
            continue;
        }
        ...
        if (!vnum1 || !vnum2) {
            fout << prim << " Ошибка: не восьмеричное число\n";
            cout << "Ошибка: не восьмеричное число\n";
            continue;
        }
        if (num1.size() > 3 || num2.size() > 3) {
            fout << prim << " Ошибка: число более трехзначного\n";
            cout << "Ошибка: число больше трехзначного\n";
            continue;
        }
        int n1 = 0, n2 = 0;
    ...
    fin.close();
    fout.close();
    cout << "Ответы записаны в файл FN2.txt" << endl;
}
